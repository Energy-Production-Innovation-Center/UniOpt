...
class Variables:
    ALLOWED_INPUT_TYPE = [tuple, list, np.ndarray]

    # TODO: Include lower and upper input
    def __init__(self, number_variables, input_values):
        self.number_variables = self.set_n_vars(number_variables)
        self.input_values = self.validate_input(input_values)
        self.var_type = self.set_variable_type(input_values)

    def set_n_vars(self, n_vars):
        if type(n_vars) is int and n_vars > 0:
            self.number_variables = n_vars
        else:
            raise ValueError(
    ...
    def validate_input(self, input_values):
        if type(input_values) not in self.ALLOWED_INPUT_TYPE:
            raise ValueError(
                f"The type of variable '{type(input_values)}' is not supported"
            )
